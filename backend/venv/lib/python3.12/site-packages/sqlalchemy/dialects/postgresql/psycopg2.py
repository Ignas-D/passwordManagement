...
* ``executemany_mode``, ``executemany_batch_page_size``,
  ...
  extensions.py for optimizing "executemany"-style queries.  See the referenced
...
2. If the ``use_native_hstore`` flag is at its default of ``True``, and
   ...
   ``psycopg2.extensions.py.register_hstore()`` extension is invoked for all
   connections.
...
``psycopg2.extensions.py.register_hstore()`` extension is not used.  It merely
